...
\begin{itemize}
\pause
\item \textbf{Основные баллы} получаются за практические задания
\pause
\item Каждая из 8-ми практик -- \textbf{до 12 баллов}
\pause
\item Можно выложить проект на гитлаб как портфолио и приложить красивый скриншот в Readme, за это ещё \textbf{4 балла}
\pause
...
\frametitle{Баллы}
\begin{itemize}
\item Дедлайн для практик -- 2 недели, 23:59 среды
\pause
\begin{itemize}
...
\pause
\item Автотестированием занимается телеграм-бот \texttt{mkn-raytracing-2025-bot} (бот начнёт работать после первой практики)
\pause
...
\pause
\item Наши программы будут довольно \textit{требовательными к производительности}, так что лучше выбрать язык пошустрее
\pause
...
\end{itemize}
\pause
\item Хорошие варианты: C, C++, Rust, и т.п.
\end{itemize}
\end{frame}
...
\end{minted}
\pause
\item Лучше делать все задания в одном репозитории, так как они основываются друг на друге
\pause
...
\pause
\begin{itemize}
\item \textbf{Лекции 1-2: Whitted-style raytracing}
\begin{itemize}
\item Простая трассировка лучей
\item Без решения уравнения рендеринга
\end{itemize}
\pause
\item \textbf{Лекции 3-5: Monte-Carlo pathtracing}
\begin{itemize}
\item Теория распространения света и модели материалов
\item Решение уравнение рендеринга
\item Оптимизация
\end{itemize}
\pause
\item \textbf{Лекции 6-8: Красивые картинки}
\begin{itemize}
\item Формат сцен glTF
\item Physically-based материалы
\item Тестуры, material mapping, environment mapping
\end{itemize}
\end{itemize}
\end{frame}
...
\pause
\begin{enumerate}
\item Описать сцену: форму объектов и их материал (напр. цвет)
\pause
\item Описать камеру: где она находится, куда смотрит
\pause
\item Задать размеры изображения
\pause
\item Для каждого пикселя изображения, построить луч из камеры в этот пиксель
\pause
\item Найти ближайшее пересечение луча с объектами сцены
\pause
\item В качестве цвета пикселя взять цвет объекта пересечения
\pause
\item Сохранить полученную картинку
\end{enumerate}
\end{frame}
...
\pause
\item Ближе к концу курса перейдём к более серьёзному формату -- \texttt{glTF}
\end{itemize}
\end{frame}
...
\end{itemize}
\pause
\item Вместо точки \begin{math}(P_X, P_Y)\end{math}, соответствующей углу пикселя, лучше брать его центр \begin{math}(P_X + 0.5, P_Y + 0.5)\end{math}
...
\pause
\item Обычно векторы направлений делают нормированными -- это упрощает очень многие вычисления, и лучше всегда придерживаться этого правила
\end{itemize}
\end{frame}
...
\pause
\item Для простоты пока будем считать, что все объекты находятся в начале координат, так что нам достаточно вектора нормали \begin{math}N = (N_X, N_Y, N_Z)\end{math}
\pause
...
\pause
\begin{itemize}
\item Вычтем \begin{math}T\end{math} из точки начала луча, и повернём результат на сопряжённый кватернион \begin{math}\overline{Q}\end{math}
\pause
...
\begin{itemize}
\item Проходим по всем объектам сцены, находим пересечение луча с ними (не забывая переводить луч в систему координат объекта)
\pause
