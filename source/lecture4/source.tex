...
\subtitle{Лекция 4: Variance reduction, importance sampling, cosine-weighted sampling, direct light sampling, multiple importance sampling}
\date{2024}
...
\begin{itemize}
\item Монте-Карло интегрирование занимает очень много времени, если мы хотим получить качественную картинку (сходимость \(\frac{1}{\sqrt N}\))
\pause
\item Хочется научиться оптимизировать этот процесс
\pause
...
\pause
\item Мат.ожидание этой величины -- искомое значение цвета пикселя (\textit{unbiased estimator})
\pause
...
= \int\limits_\Omega \frac{f(x)^2}{p(x)^2}p(x)dx - I^2 = \int\limits_\Omega \frac{f(x)^2}{p(x)}dx - \int\limits_\Omega I\cdot f(x)dx
\end{gather*}
\end{itemize}
\end{frame}
...
\item Вычислим дисперсию одного семпла:
\begin{equation*}
\operatorname{Var}\frac{f(X)}{p(X)} = \int\limits_\Omega \frac{f(x)^2}{p(x)}dx - \int\limits_\Omega I\cdot f(x)dx = \int\limits_\Omega I\cdot f(x)\cdot\left(\frac{f(x)}{I\cdot p(x)} - 1\right)dx
\end{equation*}
\pause
\item Видно, что дисперсия будет равна нулю, если
\begin{equation*}
p^*(x) = \frac{f(x)}{\int f(x)dx} = \frac{f(x)}{I}
\end{equation*}
\pause
\item \begin{math}\Longrightarrow\end{math} Поиск идеального распределения семплов сводится к интегрированию \textit{той же функции}!
\pause
\item При этом любой семпл сразу даёт точное значение интеграла:
\begin{equation*}
...
\frametitle{Variance reduction}
\begin{itemize}
\item \alert{\textbf{N.B.}}: Если \begin{math}f\end{math} бывает как положительна, так и отрицательна, то идеальное распределение находится по формуле
\begin{equation*}
p^*(x) = \frac{|f(x)|}{\int |f(x)|dx}
\end{equation*}
\pause
\item В этом случае дисперсия всегда будет больше нуля:
\begin{gather*}
...
\pause
\item Вычисление такого распределения сводится к искомому интегралу, и само по себе \textit{бесполезно}
\pause
...
\begin{frame}
\frametitle{Importance sampling: оценка variance reduction}
\begin{itemize}
\item Пусть \(p^*(x)\) -- идеальное распределение для интегрирования неотрицательной функции \(f(x)\), и \(p(x)\) -- выбранное нами распределение
\pause
\item Представим \(p(x)\) как
\begin{equation*}
p(x) = p^*(x)\cdot (1+\varepsilon(x))
\end{equation*}
\pause
\item Тогда дисперсия одного семпла равна
\begin{equation*}
\operatorname{Var}\frac{f(X)}{p(X)} = I^2 \int\limits_\Omega p^*(x) \frac{\varepsilon(x)^2}{1+\varepsilon(x)}dx
\end{equation*}
\pause
\item Если \(\varepsilon(x) \sim 0\), то и дисперсия близка к нулю
\pause
\item Если для каких-то значений \(x\) \(\quad p(x) \ll p^*(x)\), то \(\varepsilon(x) \sim -1\), и дисперсия сильно растёт из-за \(1+\varepsilon(x)\) в знаменателе
\end{itemize}
\end{frame}
...
\end{equation*}
\pause
\item Общее правило: лучше взять \(p(x)\) побольше там, где \(p^*(x)\) побольше :)
\end{itemize}
\end{frame}
...
\pause
\item К сожалению, явный вид этой функции безумно сложен и зависит от геометрии и материалов всей сцены
\pause
\item ...но мы можем попробовать какие-нибудь эвристики!
\end{itemize}
\end{frame}
...
\pause
\item Вопрос в том, как получить плотность вероятности \textit{вектора направления}, зная плотность вероятности \textit{точки поверхности}
\end{itemize}
\end{frame}
...
\pause
\item Интеграл берётся по всем точкам \(y\) всех поверхностей
\pause
...
\frametitle{Распределение направлений}
\begin{itemize}
\item От этого уравнения нам нужен только множитель \begin{math}\frac{|\cos\theta_y|}{\|x-y\|^2}\end{math}: он описывает то, какой телесный угол в точке \begin{math}x\end{math} соответствует элементу площади некой поверхности в точке \begin{math}y\end{math}
\pause
...
\begin{frame}
\frametitle{Direct light sampling: алгоритм}
\begin{itemize}
...
\frametitle{Direct light sampling}
\begin{itemize}
\item При использовании \textit{только} семплинга источников света шум значительно уменьшается
\pause
\item При этом картинка получается \textit{неправильной}, почему?
\pause
\item Мы никогда не семплим направления не на источник света, но из этих направлений может прийти непрямое (отражённое) освещение
\pause
\item Формальнее, распределение направлений не покрывает носитель интегрируемой функции, т.е. нарушается требование Монте-Карло интегрирования: \(f(x) \neq 0 \Longrightarrow p(x) \neq 0\)
\end{itemize}
\end{frame}
...
\pause
\item Direct light sampling направляет лучи в сторону источников света и полезно, когда источники маленькие или далеко, но не работает само по себе
\end{itemize}
\pause
...
\frametitle{Multiple importance sampling (MIS)}
\begin{itemize}
\item Есть много способов комбинировать распределения, подробнее смотри
\begin{itemize}
\item \href{https://cseweb.ucsd.edu/~viscomp/classes/cse168/sp21/readings/veach.pdf}{\texttt{Veach, Guibas - Optimally Combining Sampling Techniques for Monte Carlo Rendering (1995)}}
\item \href{https://graphics.stanford.edu/papers/veach_thesis/thesis.pdf}{\texttt{Veach'97 (PhD thesis)}, Chapter 9}
\item \href{https://link.springer.com/chapter/10.1007/978-1-4842-7185-8_20}{\texttt{Статья в GPU Gems II}}
\item \href{https://lisyarus.github.io/blog/posts/multiple-importance-sampling.html}{\texttt{Моя статья про MIS}}
\end{itemize}
\pause
\item Мы будем использовать простую эвристику: аффинную комбинацию нескольких распределений
\end{itemize}
\end{frame}
...
\frametitle{Multiple importance sampling (MIS)}
\begin{itemize}
\item Пусть у нас есть набор стратегий (распределений) \(p_i(x)\), и выполняется условие 
\begin{equation*}
f(x) \neq 0 \Longrightarrow \exists i:\, p_i(x)\neq 0
\end{equation*}
\pause
\item Словами: для любой точки носителя функции есть хотя бы одно распределение, которое может её сгенерировать
\pause
\item Тогда распределение \(p(x) = \sum c_ip_i(x)\), где \(c_i\) -- некоторые ненулевые веса \(\sum c_i = 1\), удовлетворяет требованиям Монте-Карло интегрирования
\pause
\item В терминах статьи Veach'а это \textit{single-sample estimator with balance heuristic} (секция 9.2.4) -- гарантированно неплохой способ комбинировать стратегии семплинга
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Multiple importance sampling (MIS): алгоритм}
\begin{itemize}
\item Алгоритм:
\pause
\begin{itemize}
\item 1. Выбираем стратегию \(i\) используя веса \(c_i\)
\pause
\item 2. Генерируем семпл с помощью стратегии \(p_i(x)\)
\pause
\item 3. Вычисляем полную вероятность \(p(x)\) семпла как взвешенную сумму всех стратегий, т.е. вычисляем вероятность того, что семпл мог быть сгенерирован \textit{любой стратегией!}
\pause
\item 4. Используем семпл в интегрировании
\end{itemize}
\end{itemize}
\end{frame}
...
\frametitle{Multiple importance sampling (MIS)}
\begin{itemize}
\item Если для всех стратегий выполняется \(f(x) \neq 0 \Longrightarrow p_i(x) \neq 0\), т.е. каждая стратегия работает сама по себе, то можно сделать проще:
\pause
\begin{itemize}
\item 1. Выбираем стратегию \(i\) используя веса \(c_i\)
\pause
\item 2. Генерируем семпл с помощью стратегии \(p_i(x)\)
\pause
\item 3. Используем этот семпл в интегрировании, с вероятностью \(p_i(x)\), т.е. игнорируя остальные стратегии
\end{itemize}
\pause
\item Такая версия MIS \textbf{не работает} для direct light sampling!
\end{itemize}
\end{frame}
...
\frametitle{Multiple importance sampling (MIS)}
\begin{itemize}
\item В нашем случае мы возьмём простую комбинацию двух распределений:
\pause
\begin{itemize}
...
\end{gather*}
\pause
\item В итоге мы сгенерируем семпл направления одним из выбранных распределений, но для Монте-Карло интегрирования нам нужна его \textit{полная} вероятность, поэтому нам всё равно нужно вычислить вероятность этого семпла для \textit{всех распределений}, а не только того, которым он сгенерирован!
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Есть другие способы улучшать сходимость Монте-Карло интегрирования, например -- \textit{stratification}
\pause
\item Разобьём область интегрирования на непересекающиеся области: \(\Omega = \bigsqcup \Omega_i\), и на каждой выберем своё распределение \(p_i\)
\pause
\item Тогда интеграл разбивается в сумму интегралов
\begin{equation*}
\int\limits_\Omega f(x)dx = \sum \int\limits_{\Omega_i} f(x)dx
\end{equation*}
\pause
\item Сгенерируем по одному семплу \(X_i\in\Omega_i\) \textit{в каждой области}, и оцениваем интеграл как
\begin{equation*}
\sum \frac{f(X_i)}{p_i(X_i)}
\end{equation*}
\end{itemize}
\end{frame}
...
\frametitle{Stratification}
\begin{itemize}
\item Дисперсия в таком случае:
\begin{equation*}
\operatorname{Var}\sum \frac{f(X_i)}{p_i(X_i)} = \sum \operatorname{Var}\frac{f(X)}{p_i(X)}
\end{equation*}
\pause
\item \(\Longrightarrow\) Метод подходит, если мы знаем хорошие стратегии для каждой области по отдельности
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Next event estimation (NEE)}
\begin{itemize}
\item Есть другие способы делать direct light sampling в рамках path tracing'а, например -- \textit{next event estimation}
\pause
\item Что-то в духе стратификации: разобьём весь приходящий свет на \textit{прямой} (т.е. излучённый источниками света) и \textit{непрямой} (т.е. отражённый хотя бы один раз)
\pause
\item При вычислении интеграла бросаем дополнительный (нерекурсивный) луч в сторону источника света для учёта прямого освещения, и генерируем луч cosine-weighted распределением (в обшем случае -- распределением, подходящим для материала) для рекурсивного луча
\pause
\item Работает очень хорошо, но сильно усложняет код (приходится учитывать ситуации, когда луч в источник света не попал в источник света, рекурсивный луч непрямого освещения попал в источник света, и т.п.), особенно с учётом отражений/преломлений
\pause
\item Подробнее смотри \href{https://www.cg.tuwien.ac.at/sites/default/files/course/4411/attachments/08_next\%20event\%20estimation.pdf}{\texttt{эту презентацию}}
\end{itemize}
\end{frame}
