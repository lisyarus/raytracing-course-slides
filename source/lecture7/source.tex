...
\pause
\begin{equation*}f(L,V) = \frac{D(H)\cdot G(L,V,H)\cdot F(L, H)}{4 \cdot (L\cdot N)\cdot(V\cdot N)}\end{equation*}
\pause
...
\pause
\item \begin{math}G(L,V,H)\end{math} -- \textit{geometry term}: насколько соседние микрофасеты мешают свету прийти от источника к поверхности (\textit{shadowing}) или от поверхности к наблюдателю (\textit{masking})
\pause
...
\usemintedstyle{solarized-light}
\begin{equation*}
D(H) = \frac{\exp\left(-\frac{\tan\theta_H}{\alpha^2}\right)}{\pi\alpha^2\cos^4\theta_H}
\end{equation*}
\begin{itemize}
...
\usemintedstyle{solarized-light}
\begin{gather*}
G_1(H,X) = \frac{1}{1 + \lambda(A)} \\
G = G_1(H,L) \cdot G_1(H,V) \\
G' = \frac{1}{1 + G_1(H,L) + G_1(H,V)}
\end{gather*}
\begin{itemize}
\item \begin{math}A = \frac{H\cdot X}{\alpha \sqrt{1 - (H\cdot X)^2}}\end{math}
